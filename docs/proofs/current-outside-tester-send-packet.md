...
This packet stays pinned to `@lambdacurry/anvil@0.1.0-alpha.5`. Do not swap the tester onto the floating `@alpha` tag.
...
3. If you changed the launcher or command, what you used instead
   - If you switched to global `anvil`, keep both the pinned `bun add -g @lambdacurry/anvil@0.1.0-alpha.5` line and the `anvil audit ...` line together in `Exact command`.
...
`bunx @lambdacurry/anvil@0.1.0-alpha.5 --version`
...
- the retained audit command keeps the pinned `0.1.0-alpha.5` local-only `--ci` spelling
