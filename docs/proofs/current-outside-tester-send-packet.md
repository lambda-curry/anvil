...
This packet stays pinned to `@lambdacurry/anvil@0.1.0-alpha.18`. Do not swap the tester onto the floating `@alpha` tag.
...
```bash
bunx @lambdacurry/anvil@0.1.0-alpha.18 audit --target . --ci --output ./anvil-audit.md
```
...
3. If you changed the launcher or command, what you used instead
   - If you switched to global `anvil`, keep both the pinned `bun add -g @lambdacurry/anvil@0.1.0-alpha.18` line and the `anvil audit ...` line together in `Exact command`.
...
`bunx @lambdacurry/anvil@0.1.0-alpha.18 --version`
...
- the retained audit command keeps the pinned `0.1.0-alpha.18` local-only `--ci` spelling
...
It keys validation off the saved packet's `Pinned CLI version`, so this retained `0.1.0-alpha.18` packet can still be checked after current `main` advances to a later package version.
