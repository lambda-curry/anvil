...
<CardGrid stagger>
	...
	<Card title="Detect drift (basic)" icon="warning">
		Flag stale path references and outdated validation dates. Full drift detection planned.
	</Card>
...
- **Coverage gaps** — patterns with no rule protection (audit only; drift-side coverage detection is planned)
- **Drift (basic)** — stale path references and outdated validation dates; glob drift and coverage drift detection are planned
