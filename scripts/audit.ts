...
const AI_FALLBACK_SUMMARY =
  "AI synthesis unavailable — showing structural results only.";

function buildAiFallbackMessage(args: ParsedArgs): string {
  const providerLine =
  ...
  return [
    ...
    providerLine,
    "",
    "The local audit completed and its report will still be written.",
    "Re-run with a working provider for AI-prioritized suggestions.",
  ].join("\n");
}
...
export function buildReport(
  ...
  if (result.auditMode === "full" && result.aiSynthesis.mode !== "ai") {
    lines.push(`> **${AI_FALLBACK_SUMMARY}**`);
    lines.push(
      "> The local scores, drift, coverage, guardrail, and remediation evidence below are complete. Re-run with a working provider for AI-prioritized suggestions.",
    );
    lines.push("");
  }
...
export async function runAudit(args: ParsedArgs): Promise<AuditResult> {
  ...
  if (auditMode === "full" && aiSynthesis.mode !== "ai") {
    logProgress(`⚠️ ${buildAiFallbackMessage(args)}`);
  }
